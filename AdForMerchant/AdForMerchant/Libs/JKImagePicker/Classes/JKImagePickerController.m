...
{
    if ([_delegate respondsToSelector:@selector(jk_imagePickerControllerDidCancel:)]) {
        [_delegate jk_imagePickerControllerDidCancel:self];
    }
}
...
- (void)startPhotoAssetsViewCell:(JKAssetsViewCell *)assetsCell
{
    if (self.selectedAssetArray.count>=self.maximumNumberOfSelection) {
        NSString  *str = [NSString stringWithFormat:@"图片已达到%lu张，不能再上传图片", (unsigned long)self.maximumNumberOfSelection];
...
- (void)resetFinishFrame
{
    ...
    self.finishLabel.text = [NSString stringWithFormat:@"确定(%ld)",self.selectedAssetArray.count];
