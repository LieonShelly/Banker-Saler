...
- (UIButton *)titleButton {
    if (!_titleButton) {
        ...
        [_titleButton setTitle:_model.name forState:normal];
        [_titleButton setTitleColor:[UIColor blackColor] forState:0];
...
- (void)viewDidLoad {
    ...
    UIButton *browBtn = [UIButton new];
    [browBtn setTitleColor:[UIColor orangeColor] forState:0];
    [browBtn setTitle:@"预览" forState:normal];
    [browBtn sizeToFit];
    [browBtn addTarget:self action:@selector(browSelectredPhoto) forControlEvents:UIControlEventTouchUpInside];
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:browBtn];
...
- (void)browSelectredPhoto {
    TZPhotoPreviewController *photoPreviewVc = [[TZPhotoPreviewController alloc] init];
    photoPreviewVc.currentIndex = self.index;
...
//    _previewButton.enabled = tzImagePickerVc.selectedModels.count;
    ...
    if (tzImagePickerVc.allowPickingOriginalPhoto) {
        ...
        [_originalPhotoButton setImage:[UIImage imageNamedFromMyBundle:tzImagePickerVc.photoOriginImageName] forState:UIControlStateNormal];
        [_originalPhotoButton setImage:[UIImage imageNamedFromMyBundle:tzImagePickerVc.photoOriginImageSelName] forState:UIControlStateSelected];
        ...
        label.text = @"原图";
        [label sizeToFit];
        label.frame = CGRectMake(_originalPhotoButton.tz_width*0.4, 0, label.tz_width, label.tz_height);
        label.tz_centerY = _originalPhotoButton.tz_height/2;
        [_originalPhotoButton addSubview:label];
        if (_isSelectOriginalPhoto) [self getSelectedPhotoBytes];
    }
    
    _okButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _okButton.frame = CGRectMake(self.view.tz_width - 75 - 12, 10, 75, 30);
    ...
    _okButton.enabled = tzImagePickerVc.selectedModels.count || tzImagePickerVc.alwaysEnableDoneBtn;
     [_okButton setTitle:[NSString stringWithFormat:@"下一步(%zd)",tzImagePickerVc.selectedModels.count] forState:normal];
    _okButton.hidden = tzImagePickerVc.selectedModels.count>0 ? NO: YES;
...
- (void)okButtonClick {
    ...
    NSLog(@"tzImagePickerVc.minImagesCount=%ld",(long)tzImagePickerVc.minImagesCount);
    NSLog(@"tzImagePickerVc.selectedModels.count=%ld",(long)tzImagePickerVc.selectedModels.count);
    NSLog(@"tzImagePickerVc.minImagesCount=%ld",(long)tzImagePickerVc.minImagesCount);
...
//    _numberLable.text = [NSString stringWithFormat:@"%zd",tzImagePickerVc.selectedModels.count];
    [_okButton setTitle:[NSString stringWithFormat:@"下一步(%zd)",tzImagePickerVc.selectedModels.count] forState:normal];
    [_okButton setTitleColor:[UIColor whiteColor] forState:0];
    _okButton.hidden = tzImagePickerVc.selectedModels.count>0 ? NO: YES;
    ...
    if (buttonIndex == 1) { // 去设置界面，开启相机访问权限
        if (iOS8Later) {
            ...
            if ([[UIApplication sharedApplication] canOpenURL:privacyUrl]) {
                ...
                UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:[NSBundle tz_localizedStringForKey:@"Sorry"] message:message preferredStyle:1];
                [self.navigationController presentViewController:alertVC animated:true completion:nil];
//                [alertVC addAction: [UIAlertAction actionWithTitle:[NSBundle tz_localizedStringForKey:@"OK"]  style:UIAlertActionStyleDefault handler:nil]];
